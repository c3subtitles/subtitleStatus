...
for any_lang in lang_dict:
   print(any_lang, lang_dict[any_lang],"\n")
   lang_liste.append([any_lang, lang_dict[any_lang]])
print("\n\n\n =========================")
for any_lang in lang_liste:
    print (any_lang[0], any_lang[1])
    Language.objects.get_or_create(lang_amara_short=any_lang[0])
    """
    this_lang = Language.objects.get_or_create(lang_amara_short = any_lang[0])
    this_lang.language_en = any_lang[1]
    this_lang.save()
    """
    """
    try:
        this_lang = Language.objects.filter(lang_amara_short = any_lang[0])
        if len(this_lang):
            this_lang[0].language_en = any_lang[1]
            this_lang[0].save()
        else:
            this_lang = Language.objects.create()
            
    except ObjectDoesNotExist:
        this_lang = Language.objects.create(lang_amra_short = any_lang[0], lang_en = any_lang[1])
    """
print ("done")