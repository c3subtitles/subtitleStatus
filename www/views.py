...
def media_export(request, timestamp, *argh, **kwargs):
    ...
    for any in my_subtitles:
        ...
        if any.state_id == 7:
            csv_output += any.talk.guid+";"+str(any.complete)+";"+any.language.lang_code_media+";"+any.language.lang_short_srt+";"+any.last_changed_on_amara.strftime("%Y-%m-%dT%H:%M:%SZ")+";"+str(any.revision)+";https://mirror.selfnet.de/c3subtitles/"+any.talk.event.subfolder_in_sync_folder+"/"+any.talk.filename+"."+any.language.lang_short_srt+ "_DRAFT" + ".srt"+";"+any.touched.strftime("%Y-%m-%dT%H:%M:%SZ")+";"+any.talk.amara_key+";"+any.language.lang_amara_short+";"+str(any.state_id)+";"+"https://amara.org/api/videos/"+any.talk.amara_key+"/languages/"+any.language.lang_amara_short +"/subtitles/"+ ";" + str(True) + "\n"
        else:
